...
export const analyzeAndSolve = async (script: string): Promise<SolutionItem[]> => {
  const model = "gemini-2.5-flash";
  // Updated prompt for Solving with Strict GDPT 2018 Rules
  const prompt = `
    Bạn là Gia Sư Hóa Học Thông Minh (GDPT 2018 Việt Nam).
    
    QUY TẮC NGHIÊM NGẶT (TUÂN THỦ TUYỆT ĐỐI):
    1. NGÔN NGỮ: 
       - Tên chất hóa học: BẮT BUỘC tiếng Anh IUPAC (Ví dụ: Iron, Copper, Nitric acid).
       - Lời giải thích: Tiếng Việt.
    
    2. PHẠM VI: 
       - Chỉ dùng kiến thức SGK Hóa 10, 11, 12 MỚI.
       - Không dùng kiến thức Đại học.

    3. XỬ LÝ PHẢN ỨNG (QUAN TRỌNG):
       - Nếu đề bài cho đủ điều kiện -> Viết 1 phương trình chính xác.
       - Nếu đề bài THIẾU điều kiện (Ví dụ: "Cho Fe tác dụng với H2SO4" mà không nói loãng hay đặc):
         -> KHÔNG được đoán mò.
         -> PHẢI chia trường hợp:
            + Trường hợp 1: H2SO4 loãng -> FeSO4 + H2.
            + Trường hợp 2: H2SO4 đặc nóng -> Fe2(SO4)3 + SO2 + H2O.
    
    Nhiệm vụ:
    - Trích xuất câu hỏi và đưa ra LỜI GIẢI CHI TIẾT.
    - Định dạng Display: Không dùng Markdown in đậm (**), dùng xuống dòng (\\n) hợp lý.
    - Định dạng Reading: Đọc tên chất bằng tiếng Anh.
    ...
    [
      { 
        ...
        "solutionDisplay": "Lời giải (Chia trường hợp nếu cần, Chuẩn IUPAC)", 
